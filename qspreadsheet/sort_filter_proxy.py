...
from qspreadsheet.common import SER
...
class DataFrameSortFilterProxy(qt.QSortFilterProxyModel):
    column_filtered = qt.Signal(int)
    column_unfiltered = qt.Signal(int)

    def __init__(self, model: DataFrameModel, parent: Optional[qt.QWidget] = None) -> None:
        ...
        self._pool = qt.QThreadPool(self)
        ...
        self.filter_cache: Dict[int, SER] = {DEFAULT_FILTER_INDEX: self.alltrues()}
    ...
    def create_filter_widget(self) -> FilterWidgetAction:
        ...
        self._filter_widget.show_all_btn.clicked.connect(self.async_refill_list)
    ...
    def string_filter(self, text: str):
        ...
        self._display_values = pd.Series(
            {
                ndx: self._model.delegate.display_data(
                    self._model.index(ndx, self._column_index), value
                )
    ...
    def filter_list_widget_by_text(self, text: str):
        ...
        if text:
            ...
            state = qt.Qt.Checked if mask.all() else qt.Qt.Unchecked
    ...
    def apply_list_filter(self, menu):
        ...
        checked = self._filter_widget.select_all_item.checkState() == qt.Qt.Checked
        ...
        if checked and is_filtered and not text:
            ...
            if not self._showing_all_display_values:
                display_values = pd.Series(
                    {ndx: value for ndx, value in self._display_values_gen}
                )
    ...
    def clear_filter_cache(self):
        if not self.is_data_filtered:
            return
        indices = [
            index for index in self.filter_cache if index != DEFAULT_FILTER_INDEX
        ]
    ...
    def populate_list(self, *args, **kwargs):
        ...
        self._display_values_gen = (
            (
                ndx,
                self._model.delegate.display_data(
                    self._model.index(ndx, self._column_index), value
                ),
            )
        ...
        if model_values.size <= INITIAL_FILTER_LIMIT:
            ...
            while next_step and self._filter_values.size < INITIAL_FILTER_LIMIT:
                ...
                to_display = pd.Series(
                    dict(next(self._display_values_gen) for _ in range(next_step))
                )
                self._display_values = self._display_values.append(to_display)
                filter_index = to_display.str.lower().drop_duplicates().index
                self._filter_values = self._filter_values.append(
                    to_display.loc[filter_index]
                )
                remaining -= next_step
        ...
        # Add a (Select All)
        if mask.all():
            select_all_state = qt.Qt.Checked
        else:
            select_all_state = qt.Qt.Unchecked
    ...
    def add_list_items(self, values: SER, checked_mask: SER):
        ...
        for row_ndx, value in values.items():
            state = qt.Qt.Checked if checked_mask.loc[row_ndx] else qt.Qt.Unchecked
            item = qt.QListWidgetItem(value)
            item.setFlags(item.flags() | qt.Qt.ItemIsUserCheckable)
    ...
    def on_rows_inserted(self, parent: qt.QModelIndex, first: int, last: int):
    ...
    def on_rows_removed(self, parent: qt.QModelIndex, first: int, last: int):
        count = last - first + 1
        for index, mask in self.filter_cache.items():
