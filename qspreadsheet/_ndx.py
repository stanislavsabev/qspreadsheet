...
class _Ndx:
    ...
    def count_committed(self) -> int:
        """Row count of 'committed' data rows,
        excluding `in progress` and `virtual` rows, if any
        """
    ...
    def _update_in_progress(self, index):
        self._data.loc[index, "in_progress"] = (
            ...
            + self._data.loc[index, "non_nullable_in_progress_count"]
            > 0
        )
    ...
    def remove(self, at_index: int, count: int):
        ...
        self._data = pandas_obj_remove_rows(self._data, at_index, count)

    def is_virtual(self, index: int) -> bool:
        return self.count_virtual and index >= self._data.index.size
    ...
    def _make_index_data_for(index) -> DF:
        ...
        return pd.DataFrame(
            data={
                ...
                "disabled": False,
                "non_nullable": False,
                "non_nullable_in_progress_count": 0,
                "disabled_in_progress_count": 0,
            },
            index=index,
        )
