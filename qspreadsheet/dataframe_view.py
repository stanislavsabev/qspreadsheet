...
from qspreadsheet.delegates import ColumnDelegate, MasterDelegate, automap_delegates
...
class DataFrameView(qt.QTableView):
    ...
    def __init__(
        self,
        df: DF,
        delegates: Optional[Mapping[Any, ColumnDelegate]] = None,
        parent=None,
    ) -> None:
        super(DataFrameView, self).__init__(parent)
        self.threadpool = qt.QThreadPool(self)
        ...
        self.header_view.filter_btn_mapper.mapped[str].connect(self.filter_clicked)
        ...
        self._proxy.column_filtered.connect(
            lambda col_ndx: self.header_view.header_widgets[col_ndx].set_filtered(True)
        )
        self._proxy.column_unfiltered.connect(
            lambda col_ndx: self.header_view.header_widgets[col_ndx].set_filtered(False)
        )

        self.horizontalScrollBar().valueChanged.connect(
            self._model.on_horizontal_scroll
        )
    ...
    def set_edit_state(
        ...
        missing = [column for column in columns if column not in self._df.columns]
        if missing:
            plural = "s" if len(missing) > 1 else ""
            raise ValueError(
                "Missing column{}: `{}`.".format(plural, "`, `".join(missing))
            )
    ...
    def filter_clicked(self, name: str):
        btn: qt.QPushButton = self.sender().mapping(name)
        ...
        menu_pos = qt.QPoint(
            header_pos.x() + self.header_menu.width() - btn.width() + 5,
    ...
    def make_cell_context_menu(self, row_ndx: int, col_ndx: int) -> qt.QMenu:
        menu = qt.QMenu(self)
        self._proxy.set_filter_key_column(col_ndx)

        # By Value Filter
        menu.addAction(
            standard_icon("CommandLink"),
            "Filter By Value",
            partial(self.filter_by_value, row_ndx, col_ndx),
        )
        ...
        menu.addAction(
            standard_icon("DialogResetButton"), "Clear Filter", self.clear_all_filters
        ).setEnabled(self._proxy.is_data_filtered)

        header_widget = self.header_view.header_widgets[col_ndx]
        menu.addAction(
            standard_icon("DialogResetButton"),
        ...
        if self._model.row_ndx.is_mutable:
            menu.addAction("Insert Rows Above", partial(self.insert_rows, "above"))
            menu.addAction("Insert Rows Below", partial(self.insert_rows, "below"))
            menu.addSeparator()
            menu.addAction("Deleted Selected Rows", self.remove_rows)
    ...
    def make_header_menu(self, col_ndx: int, header_widget: HeaderWidget) -> qt.QMenu:
        ...
        menu.addAction(
            standard_icon("TitleBarShadeButton"),
            "Sort Ascending",
            partial(self.model().sort, col_ndx, qt.Qt.AscendingOrder),
        )
        menu.addAction(
            standard_icon("TitleBarUnshadeButton"),
            "Sort Descending",
            partial(self.model().sort, col_ndx, qt.Qt.DescendingOrder),
        )
        ...
        # Hide
        menu.addAction(
            "Hide Column", partial(self.hideColumn, self._proxy.filter_key_column)
        )
        ...
        for i in (-1, 1):
            ...
            if self.isColumnHidden(ndx):
                menu.addAction(
                    f"Unhide {self._df.columns[ndx]}", partial(self.showColumn, ndx)
                )
        ...
        if hidden_indices:
            menu.addAction("Unhide All", partial(_unhide_all, hidden_indices))
        ...
        self._proxy._filter_widget.all_deselected.connect(
            action_btn_box.disableOkayButton
        )
    ...
    def insert_rows(self, direction: str):
        ...
        indexes: List[qt.QModelIndex] = self.selectionModel().selectedIndexes()
        ...
        def _insert(rows: List[int]):
            ...
            self.model().insertRows(row, len(rows), qt.QModelIndex())
    ...
    def remove_rows(self):
        ...
        num_to_delete = (
            len(rows) - self._model.row_ndx.in_progress_mask.iloc[rows].sum()
        )
        if self._model.row_ndx.count_committed - num_to_delete <= 0:
            ...
            logger.error(msg)
            qt.QMessageBox.critical(self, "ERROR.", msg, qt.QMessageBox.Ok)
            return False

        if sequential:
            self.model().removeRows(rows[0], len(rows), qt.QModelIndex())
        else:
            for row in reversed(rows):
                self.model().removeRows(row, 1, qt.QModelIndex())
    ...
    def filter_by_value(self, row_ndx: int, col_ndx: int):
        cell_val = self.model().data(
            self.model().index(row_ndx, col_ndx), qt.Qt.DisplayRole
        )
        self._proxy.set_filter_key_column(col_ndx)
    ...
    def to_excel(self, *args, **kwargs):
        ...
        try:
            ...
            logger.error("Failed to export to Excel", exc_info=e)
    ...
    def on_error(
        ...
        qt.QMessageBox.critical(self, "ERROR.", formatted, qt.QMessageBox.Ok)

    def _get_visible_column_names(self) -> list:
        return [
            self._df.columns[ndx]
            for ndx in range(self._df.shape[1])
            if not self.isColumnHidden(ndx)
        ]
...
def _rows_from_index_list(indexes: List[qt.QModelIndex]) -> Tuple[List[int], bool]:
